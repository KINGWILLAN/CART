...
function displayCartItems(products) {
  cart_container.innerHTML = cart_items
    .map(
      (item) =>
        `<div class="single-product">
               ...
               <div class="product_information">
                   ...
                   <p class="product_amount">&#8358; ${currencyDisplay.format(
                     item.price
                   )}</p>
          ...
          item.quantity
        }</span> <button onclick=increaseQty(${item.productID})>+</button></div>
                   <button onclick=removeItemFromCart(${
                     item.productID
                   }) class="remove-btn">Remove</button>
                   <button onclick=updateProductLikeness(${item.productID})>
...
function calculateCartTotal() {
  ...
  cartTotal.textContent =
    "Total = " + currencyDisplay.format(totalCostOfItemInCart);
}
